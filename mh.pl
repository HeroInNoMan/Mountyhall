...
#
# features :
# – ajouter un numéro à chaque message et une commande pour supprimer un message par son numéro ;
# – gérer les accents (le folding) dans la recherche des noms ;
#
# bugs :
# – si quelqu’un se connecte avec un nick en [xyz], la recherche
#   de matching de nick interprète à tort les parenthèses et match
#   le premier nick contenant l’un des caractères x, y ou z.
################################################################
...
my @members = ("zhou", "ruk", "kra", "tougne", "jinz", "moriss", "acete", "babass", "nano", "patch");
my %messages = ();
...
sub display_pending_messages {
  ...
  foreach my $dest (keys %messages) {
    say(color('BOLD', "Messages pour $dest :"));
    foreach my $m (@{$messages{$dest}}) {
      my %msg = %{$m};
      say(color('BOLD', " * $msg{'text'} ($msg{'sender'}, " . timestamp(@{$msg{'timestamp'}}) . ")"));
    }
  }
}
...
sub find_dests {
  ...
  my @dests = ();
  foreach my $dest (keys %messages) {
    if ($nickname =~ m/$dest/i || $dest =~ m/$nickname/i) {
      push(@dests, $dest);
    }
  }
  return @dests;
  ...
  if ($dest eq "all") {
    foreach my $member (@members) {
      unless (grep(/$member/i, @nicks)){
        push( @{ $messages{ $member } }, {'sender' => $nick, 'text' => $text, 'timestamp' => [localtime()]} );
      }
    }
    say("ok, on leur dira.");
  } else {
    if (grep(/$dest/i, @nicks)) {
      ...
      push( @{ $messages{ $dest } }, {'sender' => $nick, 'text' => $text, 'timestamp' => [localtime()]} );
      say("ok, on lui dira.");
    }
  }
}
...
sub timestamp {
  ...
  my @t = localtime();
  my $timestamp;
  unless (@t[3] == @_[3] && @t[4] == @_[4] && @t[5] == @_[5]) {
    # jour/mois/année différent, on précise la date
    @_[5]+=1900;                        # année commence à 1900
    @_[4] = sprintf("%02d", @_[4] + 1); # padding mois
    @_[3] = sprintf("%02d", @_[3]);     # padding jour
    $timestamp .= "le @_[3]/@_[4]/@_[5] ";
  }
  @_[1] = sprintf("%02d", @_[1]); # padding minutes
  $timestamp .= "à @_[2]:@_[1]";
  return $timestamp;
}
